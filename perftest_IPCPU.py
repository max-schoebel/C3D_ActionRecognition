    ...
    def run(self):
        try:
            while not self.coord.should_stop():
                ...
                start = self.examples_per_gpu
                end = start + self.examples_per_gpu
                
                # TODO: make independent of tensor naming
                data_tensor = self.graph.get_tensor_by_name('test/input_placeholder:0')
                feed_dict[data_tensor] = data
                
                label_tensor = self.graph.get_tensor_by_name('test/output_placeholder:0')
                feed_dict[label_tensor] = labels
                
                epoch_ended_tensor = self.graph.get_tensor_by_name('test/epoch_ended_placeholder:0')
...
result_filename = './pipeline_performance/ipcpu.npy'
...
with my_graph.as_default(), tf.device('/cpu:0'):
    ...
    with tf.variable_scope('test') as scope:
        ...
        gpu_queue = tf.FIFOQueue(
            GPU_QUEUES_CAPACITY, [tf.float32, tf.float32, tf.bool], name='InputQueue{}'.format(1))
        ...
        xentropy_loss, regularization_loss = C3Dmodel.loss(network_output, labels, collection='xentropy_loss', scope=None)
