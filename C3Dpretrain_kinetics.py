...
logdir = "{}/run-{}-pretrain/".format(root_logdir, now)
srcdir = logdir + 'src/'
ckptdir = "{}/run-{}-pretrain/".format(root_ckptdir, now)
...
if not os.path.exists(srcdir):
    os.makedirs(srcdir)
os.system(r'cp ./C3Dpretrain_kinetics.py {}'.format(srcdir))
...
MODEL_REPLICA = data_provider.model_replica

def queue_input_placeholders():
    # TODO: Test with variable BATCH_SIZE
    input_placeholder = tf.placeholder(
        tf.float32, [EXAMPLES_PER_GPU, MODEL_REPLICA, TEMPORAL_DEPTH, INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNELS],
            ...
            with tf.device('/gpu:%d'%i), tf.name_scope('Tower%d'%i) as scope:
                ...
                with tf.variable_scope('model_replicas'):
                    for j in range(MODEL_REPLICA):
                        ...
                        C3Dmodel.inference(
                            tf.squeeze(data_per_replica[j], axis=1), EXAMPLES_PER_GPU, dropout_placeholder,
                            is_training_placeholder, NUM_CLASSES, collection='network_output', tensor_to_return='fully')
                        )
                ...
                with tf.variable_scope('out'):
                    wout = C3Dmodel.model_variable('wout', [MODEL_REPLICA * 4096, NUM_CLASSES], C3Dmodel.weight_init, C3Dmodel.WEIGHT_DECAY)()
                    bout = C3Dmodel.model_variable('bout', [NUM_CLASSES], C3Dmodel.bias_init)()
                    out = tf.matmul(concatenated_tensor, wout) + bout
                    out = out - tf.expand_dims(tf.reduce_max(out,1),1)
                    C3Dmodel.add_activation_summary(out)
                    tf.get_variable_scope().reuse_variables()
                    # out = tf.Print(out, [tf.shape(out)], message='out', summarize=10)
                
                xentropy_loss, regularization_loss = C3Dmodel.loss(out, labels, collection='xentropy_loss', scope=scope)
                ...
                accuracy_op, accuracy_summary_op = C3Dmodel.accuracy(out, labels, collection='accuracy_op')
                tf.get_variable_scope().reuse_variables()
